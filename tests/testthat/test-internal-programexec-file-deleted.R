...
#
#   deleted audited event
#




testthat::test_that( "programexec.auditDeleted", {
  ...
  pgm <- c( "# test program",
            paste0( "base::unlink( \"", file.path( test_output_parent_ref, "output.txt", fsep = "/"), "\" )") )
...
testthat::test_that( "programexec.auditMultiDeleted", {
  ...
  test_files_to_be_deleted <- sapply( utils::head(test_references, n = 4 ), function(x) {
  ...
  for ( xpath in test_files_to_be_deleted )
    pgm <- append( pgm, 
                   paste0( "base::unlink( \"", xpath , "\" )") ) 
  ...
  # deleted
  expected_deleted <- lapply( sort( test_files_to_be_deleted ), function(x) {
    c( "path" = x,
       "sha1" = unname(ref_sha1[ x ]) )
  })
